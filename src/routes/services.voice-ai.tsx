...
import { Mic, ArrowRight, CheckCircle2, ChevronLeft, Send, Star } from "lucide-react";
...
function VoiceAIPage() {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const formData = {
        "Service": "Voice AI Agents & Speech Automation",
        "Selected Package": selectedPackage,
        "Name": form.name,
        "Email": form.email,
        "Phone": form.phone,
        "Details": form.message || "Not Specified",
        _subject: `Order Request: Voice AI (${selectedPackage} Package)`,
      };
  ...
  return (
    <div className="px-4 sm:px-6 py-16 md:py-24 max-w-7xl mx-auto animate-fade-up">
      ...
      <div className="grid lg:grid-cols-3 gap-8 items-center mb-16">
        <div className="lg:col-span-2">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 text-xs font-mono font-bold uppercase tracking-widest mb-4">
            <Mic size={14} /> Standalone AI Service Page
          </div>
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-black text-white tracking-tight mb-5 leading-tight">
            Voice AI Agents & <span className="bg-gradient-to-r from-cyan-300 via-cyan-400 to-blue-400 bg-clip-text text-transparent">Speech Automation</span>
          </h1>
          <p className="text-base sm:text-lg text-slate-300 max-w-2xl mb-8 font-medium">
            Human-like voice agents handling phone calls, booking appointments, and conducting outbound follow-up with sub-500ms latency.
          </p>
          <a href="#packages" className="btn-neon py-3.5 px-6 rounded-2xl text-xs sm:text-sm font-bold inline-flex items-center gap-2">
            View Voice Packages <ArrowRight size={16} />
          </a>
        </div>

        <div className="glass-strong rounded-3xl p-8 border border-cyan-500/40 relative bg-slate-950/80">
          <Mic size={36} className="text-cyan-400 mb-4" />
          <h3 className="text-xl font-black text-white mb-2">Telephony Voice AI</h3>
          <p className="text-xs text-slate-300 mb-6">Sub-500ms latency, ElevenLabs & Twilio integration.</p>
      ...
      <div id="packages" className="mb-20 scroll-mt-24">
        <h2 className="text-3xl font-black text-white text-center mb-12">Packages for <span className="text-cyan-400">Voice AI</span></h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div onClick={() => setSelectedPackage("Starter")} className={`glass-strong rounded-3xl p-8 border cursor-pointer ${selectedPackage === "Starter" ? "border-cyan-400 bg-slate-900/90" : "border-slate-800 bg-slate-950/80"}`}>
            <span className="text-xs font-mono text-slate-400 font-bold uppercase block mb-2">Voice Starter</span>
            <span className="text-3xl font-black text-white">$2,500</span>
            <p className="text-xs text-slate-300 my-4">Single inbound phone agent for booking & FAQs.</p>
            <div className="space-y-2 text-xs text-slate-300 mb-6">
              <div><CheckCircle2 size={14} className="text-cyan-400 inline mr-2" /> Single Inbound Phone Agent</div>
              <div><CheckCircle2 size={14} className="text-cyan-400 inline mr-2" /> Up to 5,000 Voice Mins / mo</div>
            </div>
            <button className="w-full py-3 rounded-xl bg-slate-800 text-white text-xs font-bold">Order Voice Starter ($2,500)</button>
          </div>
          ...
          <div onClick={() => setSelectedPackage("Pro")} className={`glass-strong rounded-3xl p-8 border relative scale-[1.03] cursor-pointer ${selectedPackage === "Pro" ? "border-cyan-400 bg-slate-900/95" : "border-cyan-500/60 bg-slate-950/90"}`}>
            ...
            <span className="text-xs font-mono text-cyan-400 font-bold uppercase block mb-2 mt-1">Voice Agent Pro</span>
            <span className="text-3xl font-black text-cyan-300">$5,500</span>
            <p className="text-xs text-slate-300 my-4">Inbound & outbound voice agents with live transfers.</p>
            <div className="space-y-2 text-xs text-slate-200 mb-6">
              <div><CheckCircle2 size={14} className="text-cyan-400 inline mr-2" /> Inbound & Outbound Voice Agents</div>
              <div><CheckCircle2 size={14} className="text-cyan-400 inline mr-2" /> Live Call Transfer & CRM Sync</div>
              <div><CheckCircle2 size={14} className="text-cyan-400 inline mr-2" /> Up to 25,000 Voice Mins / mo</div>
            </div>
            <button className="btn-neon w-full py-3 rounded-xl text-xs font-black">Order Voice Pro ($5,500)</button>
          </div>
          ...
          <div onClick={() => setSelectedPackage("Enterprise")} className={`glass-strong rounded-3xl p-8 border cursor-pointer ${selectedPackage === "Enterprise" ? "border-amber-400 bg-slate-900/90" : "border-amber-500/40 bg-slate-950/80"}`}>
            <span className="text-xs font-mono text-amber-400 font-bold uppercase block mb-2">Enterprise Voice Telephony</span>
            <span className="text-3xl font-black text-amber-300">$12,000+</span>
            <p className="text-xs text-slate-300 my-4">High-volume call center automation & SIP trunks.</p>
            <div className="space-y-2 text-xs text-slate-200 mb-6">
              <div><CheckCircle2 size={14} className="text-amber-400 inline mr-2" /> Dedicated SIP Trunk & PBX Server</div>
              <div><CheckCircle2 size={14} className="text-amber-400 inline mr-2" /> Custom Voice Cloning</div>
            </div>
      ...
      <div id="proposal-form" className="max-w-3xl mx-auto mb-20">
        <div className="glass-strong rounded-3xl p-8 border border-cyan-500/40 bg-slate-950/90">
          <h3 className="text-2xl font-black text-white text-center mb-6">Order Voice AI ({selectedPackage})</h3>
          {sent ? <div className="text-center py-6"><CheckCircle2 size={36} className="text-cyan-400 mx-auto mb-2" /><p className="text-white font-bold">Request Sent!</p></div> : (
            <form onSubmit={handleFormSubmit} className="space-y-4">
              ...
              <textarea rows={3} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} className="w-full px-4 py-3 bg-slate-950 border border-slate-800 rounded-xl text-white text-sm resize-none" placeholder="Call volumes & flow details..." />
