...
export function SiteHeader() {
  ...
  return (
    <>
      <header className="fixed top-6 left-1/2 -translate-x-1/2 w-[92%] max-w-7xl z-[70] glass shadow-[0_12px_40px_rgba(0,0,0,0.65)] border border-[var(--neon-cyan)]/15 rounded-full backdrop-blur-lg">
        <div className="px-8 h-[72px] flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-1.5">
            {links.map((l) => (
              <Link
                key={l.to}
                to={l.to}
                className="px-4 py-1.5 text-[15px] font-medium rounded-full text-muted-foreground hover:text-foreground hover:bg-white/5 transition-all duration-300 relative flex items-center"
                activeProps={{
                  className:
                    "px-4 py-1.5 text-[15px] font-semibold rounded-full text-foreground relative flex items-center bg-white/5 border border-[var(--neon-cyan)]/25 shadow-[0_0_15px_rgba(99,235,249,0.12)]",
                }}
          ...
          <Link
            to="/contact"
            className="hidden md:inline-flex items-center btn-neon px-5 py-2.5 rounded-full text-sm font-semibold tracking-wide hover:shadow-[0_0_20px_var(--neon-cyan)]"
          >
            Get Started
          </Link>
            ...
            <div className="flex flex-col items-center gap-6 w-full max-w-sm relative z-10">
              {links.map((l, i) => (
                <Link
                  ...
                  activeProps={{
                    className: "text-2xl font-display tracking-tight text-gradient scale-105",
                  }}
                >
                  {l.label}
                </Link>
